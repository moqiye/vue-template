<template>
  ...
  <van-popup v-model="showCascaderGrid" round position="bottom">
          <van-cascader
            v-model="formValue.grid"
            active-color="#1989fa"
            title="选择网格"
        ...
        <van-popup v-model="showCascaderEventType" round position="bottom">
          <van-cascader
        ...
        <van-popup v-model="showEventNaturePicker" position="bottom">
          <van-picker
          :columns="['杭州', '宁波', '温州', '嘉兴', '湖州']"
        ...
        <van-popup v-model="showDangerousTypePicker" position="bottom">
            <van-picker
...
const handlePickerDepartment = (item) => {
  showDeptPicker.value = true;
};

const onConfirmDept = (value) => {
  formValue.value.department = value;
  showDeptPicker.value = false;
};

const fieldValueCas = ref("");
const mapGrid = ref({});
...
const onFinishGrid = ({ value, selectedOptions }) => {
  formValue.value.grid = value;
  showCascaderGrid.value = false;
  // findParentNodes(value)
};

const changeLabelValue = (root) => {
  if (root.children !== null && root.children?.length === 0) {
    root.children = null;
  }
  root.label = root.name;
  root.value = root.id;
  root.children?.forEach((item) => changeLabelValue(item));
};

const createMap = (dataO) => {
  const data = dataO;
  const tempMap = {};
  const dfs = (root, key, value) => {
    tempMap[root[key]] = root[value];
    if (root.children !== null && root.children?.length === 0) {
      root.children = null;
    }
    root.children?.forEach((item) => dfs(item, key, value));
  };
  const tempData = {
    value: "119888a",
    label: "top0",
...
const handleCascaderEventType = (item) => {
...
const onFinishEventType = ({ value, selectedOptions }) => {
...
const handlePickerEventNature = (item) => {
  showEventNaturePicker.value = true;
};

const onConfirmEventNature = (value) => {
...
const handlePickerDangerousType = (item) => {
  showDangerousTypePicker.value = true;
};

const onConfirmDangerousType = (value) => {
...
const findParentNodes = (childCode) => {
  const results = [];
  const queue = [childCode];
  const options = casOptionsGrid.value;
  const dfs = (root, currentCode) => {
    if (root.value === currentCode) {
      ...
      root.children?.forEach((item) => dfs(item, currentCode));
    }
  };
...
const currentTaskId = ref(null); // 当前录入后返回的事件ID
const reportType = ref("1");
const onSubmit = async (values) => {};

const submitDes = async (values) => {
